...
10-core CPU, Ubuntu 24.04, R 4.6.0, \texttt{maxentcpp} 1.0.0 from CRAN)
and exclude one-time warm-up (library load). Because
\texttt{maxent\_fit()} mutates the \texttt{FeaturedSpace} object in
place, repeated fits on the \emph{same} object restart from the
already-converged solution and massively understate per-fit cost; all
timings therefore use a fresh featured space per fit.
...
44 features, \texttt{max\_iter\ =\ 500}; median over fresh-state runs):
...
\bottomrule\noalign{}
\endlastfoot
\texttt{maxentcpp} & \textasciitilde18.1 s & \textasciitilde158 MB \\
\texttt{maxnet} & \textasciitilde0.25 s & \textasciitilde358 MB \\
\end{longtable}
}

The \texttt{maxentcpp} fit runs 261 iterations to convergence (loss
improvement \textless{} \(10^{-5}\)); the end-to-end
\texttt{maxent\_run()} workflow adds \textasciitilde2-3 ms each for
evaluation, contributions, and permutation importance, so the fit itself
accounts for \textgreater99\% of wall time. \texttt{maxnet} is faster on
small datasets because \texttt{glmnet}'s coordinate descent is highly
optimized for dense feature matrices, but it materializes the full
prediction matrix in memory: on the same data \texttt{maxentcpp} peaked
at \textasciitilde158 MB vs \textasciitilde358 MB for \texttt{maxnet}
(measured with \texttt{/usr/bin/time\ -v}, warm-up excluded) --- a
\textasciitilde2.3\(\times\) advantage that grows with raster size.
Streaming projection onto a 23,000-cell grid took 21 ms
(\texttt{maxentcpp}) vs 9 ms (\texttt{maxnet}); both are trivial at this
scale, and \texttt{maxentcpp}'s block-by-block evaluation is designed to
avoid loading rasters larger than available RAM, which we note as future
benchmark work on this hardware.

Scaling with sample size (fresh-state fits, \texttt{maxentcpp}): 2,371
cells \textasciitilde18.1 s; 23,000 cells \textasciitilde129 s
(signal-strong synthetic data exhausts the 500-iteration budget;
real-world convergence is typically earlier). A 233,000-cell run exceeds
this machine's per-run budget and is noted as future work.
...
quadratic + hinge features; \texttt{max\_iter\ =\ 500}, fresh state per
fit):
...
\bottomrule\noalign{}
\endlastfoot
Single fit (continuous only) & \textasciitilde22 s \\
Fit with categorical predictor & \textasciitilde18 s \\
Jackknife (3 variables; 6 fits) & \textasciitilde63 s \\
Cross-validation (\(k = 5\); 5 fits) & \textasciitilde135 s \\
Replicate runs (bootstrap, \(n = 5\); 5 fits) & \textasciitilde102 s \\
\end{longtable}
}
...
are linear in the number of constituent fits. Full reproducible code is
