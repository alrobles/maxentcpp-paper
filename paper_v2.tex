...
\texttt{maxentcpp} 1.0.0 installed from source) and exclude one-time
warm-up (library load). Because \texttt{maxent\_fit()} mutates the
...
\bottomrule\noalign{}
\endlastfoot
\texttt{maxentcpp} (Sequential, default) & \textasciitilde7.8 ms \\
\texttt{maxnet} & \textasciitilde270 ms \\
\end{longtable}
}
...
34 times faster than \texttt{maxnet} on this benchmark while preserving
per-iteration trajectory parity with Java Maxent 3.4.4.
...
\texttt{max\_iter\ =\ 500}, fresh state per fit; median over 20 runs):
...
\bottomrule\noalign{}
\endlastfoot
Single fit (continuous only) & \textasciitilde7.8 ms \\
Jackknife (3 variables; 6 fits) & \textasciitilde30 ms \\
Cross-validation (\(k = 5\); 5 fits) & \textasciitilde58 ms \\
Replicate runs (bootstrap, \(n = 5\); 5 fits) & \textasciitilde54 ms \\
\end{longtable}
}
