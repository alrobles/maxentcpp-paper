...
\texttt{maxentcpp} is an R package that provides a native C++17
...
Since its introduction, Maxent and its associated methodological papers
have been cited tens of thousands of times across ecology, conservation
biology, and biogeography \cite{Elith2011}.
...
improve integration with modern workflows, and retain scientific
...
\texttt{maxentcpp} contributes to this modernization pattern by targeting
a stricter compatibility goal than most wrapper packages: preserving the
Java Maxent \texttt{density.Sequential} optimizer trajectory rather than
only reproducing final predictions.
To our knowledge, \texttt{maxentcpp} is the first R package to target
the Java Maxent~3.4.4 \texttt{density.Sequential} optimizer through a
native C++ port and to publish per-iteration trajectory tests against the
Java implementation via the companion package
