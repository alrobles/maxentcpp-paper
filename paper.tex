...
measured on the benchmark machine used throughout this study (Intel
Core Ultra 7 165H, 62~GiB RAM, R~4.6.0, 30 fresh-state repetitions with
a fresh featured space per fit):
...
\hline
\texttt{maxentcpp} (Sequential, default) & $\sim$5.0\,ms \\
\hline
\texttt{maxnet}                            & $\sim$246\,ms \\
\hline
\end{tabular}
\label{tab:benchmark}
\end{table}
...
\texttt{maxentcpp} is now approximately 49~times faster than
\texttt{maxnet} on this fixture, while preserving the per-iteration
trajectory parity validated against Java Maxent~3.4.4.
\texttt{maxnet} remains an excellent alternative when a pure-R
\texttt{glmnet} path is preferred, but it uses a different optimization
