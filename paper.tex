...
measured on an Intel Xeon VM (R~4.1.2, 100 \texttt{microbenchmark}
repetitions with a fresh featured space per fit):
...
\hline
\texttt{maxentcpp} (Sequential, default) & $\sim$7.8\,ms \\
\hline
\texttt{maxnet}                            & $\sim$270\,ms \\
\hline
\end{tabular}
\label{tab:benchmark}
\end{table}
...
\texttt{maxentcpp} is now approximately 34~times faster than
