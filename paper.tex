...
measured on an Intel Xeon VM (R~4.1.2, \texttt{maxentcpp} 1.0.0,
\texttt{maxnet} 0.1.4, \texttt{dismo} 1.3-14, \texttt{predicts}
0.2-2; \texttt{microbenchmark} repetitions with a fresh featured space
per fit):
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{2.6cm}>{\raggedright\arraybackslash}p{2.8cm}>{\raggedleft\arraybackslash}p{1.3cm}>{\raggedleft\arraybackslash}p{1.1cm}@{}}
\toprule
\textbf{Package} & \textbf{Backend} & \textbf{Time} & \textbf{AUC} \\
\midrule
\texttt{maxentcpp} (Sequential, default) & C++17 \texttt{Sequential} (Eigen/BLAS) & $\sim$6.3\,ms & 0.8033 \\
\texttt{dismo} & Java Maxent \texttt{maxent.jar} & $\sim$220\,ms & 0.8048 \\
\texttt{predicts} & Java Maxent \texttt{maxent.jar} (via \texttt{rJava}) & $\sim$220\,ms & 0.8048 \\
...
After switching the default \texttt{maxent\_fit()} backend from the legacy
\texttt{goodAlpha} optimizer to the Java-faithful \texttt{Sequential}
optimizer and vectorizing the hot loops with Eigen/BLAS,
\texttt{maxentcpp} is now approximately 35--50~times faster than the other
R implementations on this fixture, while preserving the per-iteration
trajectory parity validated against Java Maxent~3.4.4. The Java-based
wrappers pay a per-call JVM startup and file-serialization cost, and
\texttt{maxnet} uses a different coordinate-descent optimizer; all four
produce statistically equivalent AUCs.
\texttt{maxentcpp}'s streaming evaluation also avoids materializing the
full prediction matrix, which is expected to reduce peak memory during
...
\hline
Categorical variables        & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\hline
Replicate runs / cross-val.  & $\checkmark$ & $\checkmark$ & via ENMeval \\
\hline
Jackknife variable selection & $\checkmark$ & $\checkmark$ & --- \\
\hline
Missing data handling        & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\hline
...
transformations.
Recent releases add categorical predictors, replicate runs and
cross-validation, jackknife diagnostics, and missing-data handling,
closing most of the remaining gaps with Java Maxent~3.4.4 for the
...
distribution models.
The recently added support for categorical predictors, replicate
diagnostics, and missing-data handling brings the package closer to
parity with Java Maxent~3.4.4 for the implemented feature classes.
