...
\cite{Phillips2006,Elith2011}, with foundational papers cited tens of
...
application \cite{Phillips2017}, and every species run in a typical
workflow --- e.g., 500 species \(\times\) 4 climatic scenarios ---
...
equivalent \texttt{maxentcpp} pipeline. On the On the full grid the two
implementations agree to numerical precision (RMSE
\textasciitilde{}\(6 \times 10^{-10}\), more than eight orders of
...
\caption{Predictions of Java Maxent 3.4.4 vs \texttt{maxentcpp} on
identical training data; the 1:1 line is overlaid in
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.0565}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.2177}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1935}}
...
\(r\) vs true (\texttt{maxentcpp})
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
\(r\) vs true (\texttt{maxnet})
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
\(r\) (\texttt{maxentcpp} vs \texttt{maxnet})
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
correlation; (right) agreement between \texttt{maxentcpp} and
\texttt{maxnet} by sample size and correlation.}\label{fig:simsum}
\end{figure}
...
All timings below were measured on the benchmark machine used throughout
this study (Intel Core Ultra 7 165H, 62 GiB RAM, Ubuntu 24.04, R 4.6.0,
\texttt{maxentcpp} 1.0.0 installed from the development source tree,
\texttt{maxnet} 0.1.4, \texttt{dismo} 1.3-14, \texttt{predicts} 0.2-2)
and exclude one-time warm-up (library / JVM load). Because
\texttt{maxent\_fit()} mutates the \texttt{FeaturedSpace} object in
place, every timing uses a fresh \texttt{FeaturedSpace} (and fresh
feature objects) per fit. The default \texttt{maxent\_fit()} backend has
been switched from the legacy \texttt{goodAlpha} optimizer to the
Java-faithful \texttt{Sequential} optimizer, and the \texttt{O(n)} hot
loops in \texttt{Sequential} are now vectorized with Eigen/BLAS.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{maxentcpp} (Sequential, default) & 1.0.0 & C++17
\texttt{Sequential} (Eigen/BLAS) & \textasciitilde6.0 ms & 0.8035 &
121 \\
\texttt{predicts} & 0.2-2 & Java Maxent \texttt{maxent.jar} (via
\texttt{rJava}) & \textasciitilde152 ms & 0.7735 & 280 \\
\texttt{dismo} & 1.3-14 & Java Maxent \texttt{maxent.jar} &
\textasciitilde163 ms & 0.7735 & 280 \\
\texttt{maxnet} & 0.1.4 & \texttt{glmnet} elastic-net &
\textasciitilde256 ms & 0.7816 & --- \\
\end{longtable}
...
contribution, and permutation importance, so the fit itself accounts for
\textgreater99\% of wall time. After the backend switch and
vectorization, \texttt{maxentcpp} is now approximately 25--43 times
faster than the other R implementations on this fixture
(\textasciitilde43\(\times\) vs \texttt{maxnet},
\textasciitilde27\(\times\) vs \texttt{dismo},
\textasciitilde25\(\times\) vs \texttt{predicts}) while preserving
per-iteration trajectory parity with Java Maxent 3.4.4 (see Numerical
fidelity). The Java-based wrappers pay a per-call JVM startup and
file-serialization cost that \texttt{maxentcpp} eliminates;
...
dataset (2.3\(\times\) less than \texttt{maxnet}'s \textasciitilde358
MB); large-raster scaling measurements on grids larger than available
RAM remain future work. The streaming raster-evaluation engine and the
\texttt{O(n)} auxiliary memory in the periodic parallel update keep the
...
function; coordinate ascent \cite{Phillips2006}, with convergence of
the underlying block-coordinate-descent flavor analyzed by
\cite{Tseng2001}), then computes a Newton step:
...
\texttt{LinearFeature} maps to \texttt{density.LinearFeature},
\texttt{FeaturedSpace} maps to \texttt{density.FeaturedSpace},
\texttt{Sequential::run()} maps to \texttt{density.Sequential.run()}),
...
family, IEEE 754 double precision, deterministic iteration order). The
Eigen/BLAS-vectorized reductions used by the default \texttt{Sequential}
backend are mathematically identical to the scalar loops but not
bit-identical: vectorized summation order can shift individual
trajectory checkpoints at the \(\sim 10^{-10}\)--\(10^{-5}\) level
depending on platform BLAS, and the C++ streaming-parity tests therefore
compare trajectories at an absolute/relative tolerance of \(10^{-5}\)
while loss and entropy remain at machine precision. Floating-point
ordering differences introduced by alternative BLAS backends or
aggressive compiler optimizations (e.g., \texttt{-ffast-math}) could
widen the gap, though the sequential nature of the coordinate-ascent
...
framework. The human author made all core design decisions, reviewed and
edited all AI-generated code, and can independently explain and modify
every algorithmically significant class (the optimizer methods are
documented with line-by-line references to the corresponding Java
source). The full development history is publicly available in the four
