...
maxentcpp is an R package offering a native C++17 reimplementation of the Maximum Entropy (Maxent) algorithm for species distribution modeling (SDM). Maxent estimates the geographic distribution of species from presence-only occurrence records and environmental covariates by identifying the probability distribution of maximum entropy subject to constraints derived from training data [@Phillips2006; @Phillips2004]. Since its introduction, Maxent and its foundational methodological papers have been cited extensively in ecology, conservation biology, and biogeography [@Elith2011].
...
$\lambda_j$ that maximizes the regularized log-likelihood via sequential
coordinate ascent with $\ell_1$ (lasso) penalties
[@Phillips2006; @Elith2011].

The original implementation of Maxent software is a Java desktop application
...
A raster evaluation engine that processes environmental layers block-by-block through the terra package [@Hijmans2024], enabling continental-scale
projections on commodity hardware without loading entire raster stacks
into memory. Tools for diagnosing model performance include AUC evaluation, permutation significance,
response curves, and multivariate environmental similarity surfaces
(MESS) for anomaly detection.

Statement of need

Maxent is the de facto standard for presence-only species distribution
modeling [@Merow2013]. However, the Java implementation presents
a number of practical barriers for present-day ecological research workflows:
...
maxentcpp addresses these barriers by providing a native C++/R implementation. The development version is currently available from GitHub using remotes::install_github("alrobles/maxentcpp"); after CRAN acceptance, installation will be possible via install.packages("maxentcpp"). The package operates entirely in memory through R objects and integrates with the terra spatial data ecosystem. The intended users are ecologists, conservation biologists, and biogeographers who employ Maxent in reproducible, script-based workflows, ranging from individual species analyses to large-scale biodiversity assessments.
...
ecosystem. The target audience is
...
modernized for current computing environments. Reimplementing legacy scientific software in modern languages is uncommon in ecology but has
...
integration with present-day workflows, and retain scientific comparability
with established software.

maxnet [@Phillips2024maxnet] represents the original MaxEnt author’s own reimplementation using R and glmnet — preserving the same statistical model but employing a different optimization algorithm (coordinate descent on the full feature matrix rather than sequential coordinate ascent).

maxentcpp advances this modernization trend by pursuing a stricter compatibility objective than most wrapper packages: it preserves the Java Maxent density.Sequential optimizer trajectory, rather than solely reproducing final predictions. To the best of current knowledge, maxentcpp is the first R package to target the Java Maxent 3.4.4 density.Sequential optimizer through a native C++ port and to publish per-iteration trajectory tests against the Java implementation via the companion package maxentcppCompTest [@maxentcppCompTest2025].
stricter compatibility goal than most wrapper packages: preserving the
Java Maxent density.Sequential optimizer trajectory rather than only
reproducing final predictions. To our knowledge, maxentcpp is the first R package to target the Java Maxent 3.4.4 density. Sequential
optimizer through a native C++ port and to publish per-iteration
trajectory tests against the Java implementation via the companion
package maxentcppCompTest [@maxentcppCompTest2025].
...
coordinate-ascent optimizer of the original one Maxent, which can produce
...
wraps either dismo::maxent() or maxnet::maxnet(); it does not implement the Maxent algorithm itself.
...
To quantify the practical differences between maxentcpp and maxnet, we compared both packages on a virtual species [@Leroy2016] with known
...
The species has a Gaussian niche centered at 20 °C and 1,500 mm, and 100 presence records were sampled proportionally to the true suitability
surface (see the companion vignette Virtual Species Comparison
...
Pearson $r$	0.97
Mean $	\Delta\text{cloglog}
Max $	\Delta\text{cloglog}
...
A more extensive simulation study across multiple species, sample sizes, and predictor correlation structures is required to establish general equivalence.

The key applicable scenarios where a user must choose maxentcpp over
maxnet are: (1) when the exact reproduction of Java Maxent results is
required (e.g., replicating published analyses), (2) when the lambda file
interoperability with Java Maxent is needed, and (3) when streaming raster projection onto grids larger than available RAM is required.
...
maxent_run() function provides a one-call workflow emulating the Java
...
parameter with $\hat\sigma_j$ being the standard deviation of the feature
...
$\partial \mathcal{L}/\partial \lambda_j = \mathbb{E}q[f_j] -
\mathbb{E}{\hat p}[f_j] + \beta_j ,\mathrm{sign}(\lambda_j)$.
...
tile, then normalizing once after all tiles are processed. This produces results equivalent to single-pass evaluation because the mathematical sum is commutative and each cell is scored independently; floating-point summation order may introduce differences at the
...
This design decision prioritizes backward compatibility: users migrating from Java Maxent can expect numerically equivalent results for implemented features under matched inputs, defaults, and deterministic settings. This fidelity contract is enforced by a dedicated companion package, maxentcppCompTest [@maxentcppCompTest2025], which runs the C++ optimizer and the original Java density.Sequential on shared test fixtures and compares per-iteration trajectories of loss, entropy, and $\lambda$ vectors.
...
These limits hold under the specific conditions of the test fixtures
...
(e.g., -fast-math) might widen the gap, though the sequential
The nature of the coordinate ascent algorithm limits sensitivity to
...
interoperability with current workflows and archived models.
...
1. alrobles/Maxent — a fork of the mrmaxent/Maxent
...
The design of maxentcpp explicitly aims to preserve this comparability.
maxentcpp is explicitly designed to preserve this comparability.
